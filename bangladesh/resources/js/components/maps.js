...
const ToolTipContent = ({ data, geo, config }) => {
    ...
    const filterData = data.filter((x) =>
        x?.[config.locations.level2].toLowerCase() === geo.ADM4_EN.toLowerCase()
    );
    ...
    */
    const { ADM4_EN, UNIT_TYPE, WARD } = geo;
    return (
            ...
            >
                ...
                >
                    <Geographies geography={geoUrl}>
                        {({ geographies }) =>
                            geographies.map((geo) => {
                                const name = geo.properties?.ADM4_EN;
                                ...
                                if (level1) {
                                    enableMapOnClick =
                                        geo.properties?.ADM2_EN.toLowerCase() ===
